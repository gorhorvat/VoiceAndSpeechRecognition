...
import VerificationProfileCreationResponse
import VerificationEnrollmentResponse
...
class VerificationServiceHttpClientHelper:
    ...
    def create_profile(self, locale):
        ...
        try:
            ...
            if res.status == self._STATUS_OK:
                # Parse the response body
                return VerificationProfileCreationResponse.VerificationProfileCreationResponse(json.loads(message))
            else:
    ...
    def enroll_profile(self, profile_id, file_path):
        ...
        try:
            ...
            if res.status == self._STATUS_OK:
                # Parse the response body
                return VerificationEnrollmentResponse.VerificationEnrollmentResponse(json.loads(message))
            else:
                reason = res.reason if not message else message
